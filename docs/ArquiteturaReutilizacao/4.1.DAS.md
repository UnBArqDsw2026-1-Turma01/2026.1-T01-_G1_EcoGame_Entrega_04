...
│      Mapa       │──────────────▶│     AmbienteFacade   │
│─────────────────│               │──────────────────────│
│ _observadores   │               │ _iluminacao          │
│ _diaAtual       │               │ _ciclo               │
│ _pool: LixoPool │               │ _clima               │
│─────────────────│               │──────────────────────│
│ Attach()        │               │ AmanhecerEnsolarado()│
│ Detach()        │               │ Anoitecer()          │
│ AvancarDia()    │               │ IniciarChuva()       │
│ SpawnLixo()     │               │ EhSeguroParaColetar()│
│ ColetarLixo()   │               └──────────────────────┘
└────────┬────────┘
         │ notifica
         ▼
...
│──────────────────────────│      │─────────────────────────────────│
│ instanciaJogador (static)│      │ _instancia (static)             │
│ _nome, _velocidade       │      │ _inventario: Inventario         │
│ _energia, _moedas        │      │ _player: Player                 │
│──────────────────────────│      │─────────────────────────────────│
│ GetInstancia() : Player  │◀─────│ GetPlayer()                     │
│ ColetarItem(nomeItem)    │      │ AdicionarItemNoInventario()     │
│ AplicarBuffVelocidade()  │      │ AdicionarMoedasAoPlayer()       │
│ [Signal] ItemColetado    │─────▶│ OnPlayerItemColetado()          │
└──────────────────────────┘      └────────────────┬────────────────┘
                                                   │ «uses»
                                          ┌────────▼────────┐
                                          │    Inventario   │
