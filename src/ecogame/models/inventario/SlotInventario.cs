...
public class SlotInventario : Item
{
    ...
    public SlotInventario(int indice) : base("Slot", 0, MaterialBase.OUTROS)
    {
        _indice = indice;
    }

    public override string GetNome() => _itens.Count > 0 ? _itens[0].GetNome() : "Vazio";
    public override MaterialBase GetMaterial() => _itens.Count > 0 ? _itens[0].GetMaterial() : MaterialBase.OUTROS;
    public override int GetPontos() => _itens.Sum(item => item.GetPontos());
    public override int GetQuantidade() => _itens.Sum(item => item.GetQuantidade());

    public bool AdicionarItem(Item item)
    {
        if (EstaLotado()) return false;
        if (_itens.Count > 0 && _itens[0].GetNome() != item.GetNome()) return false;
        _itens.Add(item);
        return true;
    }
